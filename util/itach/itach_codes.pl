...
foreach $label (sort keys %tv) {
   ...
   printf("static struct irTx irTx%s\[REMOTE_MODES] = {\n", $label );
   for ($k=0; $k<3; $k++) {
      $nl = $k==2 ? "\n" : ",\n\n";
      $cr = $k==2 ? "\\r\"\n" : "\\r\",\n\n";
      chomp $cmds[$k];
      ...
      if ($cmds[$k] eq "NULL") {
         ...
         $id++;
         for ($l=0; $l < length($cmds[$k]);$l+=68) {
            $line = substr($cmds[$k], $l, 68);
            print "      \"".$line;
            print $l+68>=length($cmds[$k]) ? "\\r\",\n\n" : "\"\n";
         }
      }
      
      # append mode if command is mode specific 
